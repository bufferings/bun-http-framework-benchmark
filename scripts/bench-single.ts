...
const testEndpoint = async (endpointType: string) => {
	if (endpointType === 'ping') {
		...
		if (queryText !== '1 bun')
			throw new Error(`Result not match (expected "1 bun", got "${queryText}")`)
		...
		if (bodyText !== expectedBody)
			throw new Error(`Result not match (expected "${expectedBody}", got "${bodyText}")`)
		...
		if (responseText !== expectedResponse)
			throw new Error(`Result not match (expected "${expectedResponse}", got "${responseText}")`)
