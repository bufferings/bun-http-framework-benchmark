...
const testEndpoint = async (targetIp: string, endpointType: string) => {
  if (endpointType === "ping") {
    ...
    if (text !== expected) {
      throw new Error(
        `Result not match (expected "${expected}", got "${text}")`,
      );
    }
...
const main = async () => {
  ...
  await Bun
    ...
    .quiet();
  await Bun
    .$`gcloud compute scp --recurse --internal-ip --zone=${gcpZone} --project=${gcpProjectId} ./src ./package.json ./bun.lockb ${targetVmName}:~/bun-http-framework-benchmark/`
    .quiet();
  ...
  console.log(
    'Run "bun scripts/report-single.ts results/single-2vm.json docs/bench-single-2vm" to generate documentation',
  );
};
