...
import { generateDocs, type Results } from './tools/generate-docs'

const args = Bun.argv.slice(2)

if (args.length < 2) {
	console.error('Usage: bun scripts/report.ts <results-file> <output-dir>')
	console.error('Example: bun scripts/report.ts results/single.json docs/bench-single')
	process.exit(1)
}
