...
import {
  generateMultiDocs,
  type MultiResults,
} from "./tools/generate-docs-multi";

const args = Bun.argv.slice(2);

if (args.length < 2) {
  console.error(
    "Usage: bun scripts/report-multi.ts <results-file> <output-dir>",
  );
  console.error(
    "Example: bun scripts/report-multi.ts results/multi.json docs/bench-multi",
  );
  process.exit(1);
}
