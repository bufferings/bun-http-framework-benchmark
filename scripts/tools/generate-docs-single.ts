...
const generateRelativeSVG = (
  ...
  frameworksByRuntime.forEach((runtimeFrameworks) => {
    runtimeFrameworks.forEach((fw) => {
      if (
        currentLineWidth + itemWidth > maxLegendWidth && currentLineWidth > 0
      ) {
        legendLines++;
  ...
  const getX = (index: number) =>
    chartLeft + (chartWidth / (tests.length)) * (index + 0.5);
  ...
  for (let i = 0; i < tests.length; i++) {
    if (i % 2 === 1) {
      ...
      svg += `  <rect class="stripe" x="${x}" y="${chartTop}" width="${
        chartWidth / tests.length
      }" height="${chartHeight}"/>\n`;
    }
  }
  ...
  for (let i = 0; i <= 5; i++) {
    ...
    svg +=
      `  <line class="grid-line" x1="${chartLeft}" y1="${y}" x2="${chartRight}" y2="${y}"/>\n`;
  }
  ...
  for (const fw of frameworks) {
    ...
    data.forEach((val, i) => {
      ...
      if (markerClass === "circle") {
        svg +=
          `  <circle class="marker" cx="${x}" cy="${y}" r="7" stroke="${color}"/>\n`;
      } else if (markerClass === "rect") {
        svg += `  <rect class="marker" x="${x - 7}" y="${
          y - 7
        }" width="14" height="14" stroke="${color}"/>\n`;
        ...
        const x3 = x + 7, y3 = y + 6;
        svg +=
          `  <polygon class="marker" points="${x1},${y1} ${x2},${y2} ${x3},${y3}" stroke="${color}"/>\n`;
      }
    });
  }
  ...
  frameworksByRuntime.forEach((runtimeFrameworks, runtime) => {
    runtimeFrameworks.forEach((fw) => {
      ...
      if (markerClass === "circle") {
        svg +=
          `    <circle class="marker" cx="${legendX}" cy="${legendY}" r="7" stroke="${color}"/>\n`;
      } else if (markerClass === "rect") {
        svg += `    <rect class="marker" x="${legendX - 7}" y="${
          legendY - 7
        }" width="14" height="14" stroke="${color}"/>\n`;
      } else if (markerClass === "triangle") {
      ...
      svg += `    <text class="label" x="${legendX + 15}" y="${
        legendY + 5
      }" font-size="14">${fw}</text>\n`;
...
| Runtime | Framework      |`;
  ...
  frameworks.forEach((fw) => {
    ...
    tests.forEach((test) => {
      ...
      if (val) {
        md += ` ${
          val.toFixed(0).padStart(9).replace(/\B(?=(\d{3})+(?!\d))/g, ",")
        } |`;
      } else {
        md += ` ${"-".padStart(9)} |`;
      }
...
| Settings | ${results.meta.benchmark.duration}s duration, ${results.meta.benchmark.connections} connections, ${results.meta.benchmark.runs} run${
    results.meta.benchmark.runs > 1 ? "s" : ""
  } |`;
...
|---|---|`;
    ...
    if (results.meta.environments.target?.runtimes) {
      const runtimesStr = Object.entries(
        results.meta.environments.target.runtimes,
      )
        .map(([name, version]) =>
          `${name.charAt(0).toUpperCase() + name.slice(1)} ${version}`
        )
