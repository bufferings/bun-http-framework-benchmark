...
export const generateMultiDocs = (results: MultiResults, outputDir: string) => {
  ...
  const sortedBenchmarks = [...results.benchmarks].sort(
...
const generateMultiRelativeSVG = (
  ...
  frameworksByRuntime.forEach((runtimeFrameworks) => {
    runtimeFrameworks.forEach((fw) => {
      if (
        currentLineWidth + itemWidth > maxLegendWidth &&
        currentLineWidth > 0
      ) {
        legendLines++;
  ...
  for (const endpointName of endpointNames) {
    const values = benchmarks.map(
      (b) => b.endpoints[endpointName]?.median || 0,
    );
  ...
  const getX = (index: number) =>
    chartLeft + (chartWidth / endpointNames.length) * (index + 0.5);
  ...
  for (let i = 0; i < endpointNames.length; i++) {
    if (i % 2 === 1) {
      ...
      svg += `  <rect class="stripe" x="${x}" y="${chartTop}" width="${
        chartWidth / endpointNames.length
      }" height="${chartHeight}"/>\n`;
    }
  }
  ...
  for (let i = 0; i <= 5; i++) {
    const y = chartBottom - (chartHeight / 5) * i;
    const label = i * 20;
    svg += `  <text class="label" x="${chartLeft - 10}" y="${
      y + 5
  ...
  endpointNames.forEach((name, i) => {
    ...
    svg += `  <text class="label" x="${x}" y="${
      chartBottom + 25
    }" text-anchor="middle">${numberSymbols[i]}</text>\n`;
  });
  ...
  for (const b of benchmarks) {
    ...
    data.forEach((val, i) => {
      ...
      if (markerClass === "circle") {
        svg +=
          `  <circle class="marker" cx="${x}" cy="${y}" r="6" stroke="${color}"/>\n`;
      } else if (markerClass === "rect") {
        svg += `  <rect class="marker" x="${x - 6}" y="${
          y - 6
        }" width="12" height="12" stroke="${color}"/>\n`;
  ...
  frameworksByRuntime.forEach((runtimeFrameworks, runtime) => {
    runtimeFrameworks.forEach((fw) => {
      ...
      if (markerClass === "circle") {
        svg +=
          `    <circle class="marker" cx="${legendX}" cy="${legendY}" r="6" stroke="${color}"/>\n`;
      } else if (markerClass === "rect") {
        svg += `    <rect class="marker" x="${legendX - 6}" y="${
          legendY - 6
        }" width="12" height="12" stroke="${color}"/>\n`;
      } else if (markerClass === "triangle") {
        const x1 = legendX,
